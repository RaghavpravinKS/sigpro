...
#include "audio_sine_wave_private.h"
#include "rtwtypes.h"
#include "limits.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(audio_sine_wave_M) == (NULL));
  while (runModel) {
    MW_sem_wait(&baserateTaskSem);
    audio_sine_wave_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(audio_sine_wave_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(audio_sine_wave_M, "stopping the model");
  runModel = 0;
}
...
{
  int argc = 4;
  char *argv[4] = { "audio_sine_wave", "-w", "-port", "17725" };

  my_android_init(pEnv, pObj, pArgv, pMainActivity);
  rtmSetErrorStatus(audio_sine_wave_M, 0);

  /* Initialize model */
  audio_sine_wave_initialize();
