...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(reciever_M) == (NULL));
  while (runModel) {
    MW_sem_wait(&baserateTaskSem);
    reciever_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(reciever_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(reciever_M, "stopping the model");
  runModel = 0;
}
...
{
  int argc = 4;
  char *argv[4] = { "reciever", "-w", "-port", "17725" };

  my_android_init(pEnv, pObj, pArgv, pMainActivity);
  rtmSetErrorStatus(reciever_M, 0);

  /* Initialize model */
  reciever_initialize();

  /* Call RTOS Initialization function */
  myRTOSInit(2.2675736961451248E-5, 0);
